...
public class BookRestServiceTest {
	...
	@Test
	public void testShouldGetSearchResult() throws Exception {
		...
		// register response for bookService.searchByTitleOrAuthors() mock
		Mockito.when(bookService.searchByTitleOrAuthors(title, author)).thenReturn(Arrays.asList(bookTo1));
		// when
		ResultActions response = this.mockMvc.perform(get("/books-search").param("titlePrefix", title).param("authorPrefix", author)
	...
	@Test
	public void testShouldGetSearchResultWithSearchTo() throws Exception {

		// given
		String title = "title";
		String author = "author";
		final BookTo bookTo1 = new BookTo(1L, title, author, BookStatus.FREE);
		File file=FileUtils.getFileFromClasspath("classpath:pl/spring/demo/web/json/searchTo.json");
		String jsonSearchTo=FileUtils.readFileToString(file);
		// register response for bookService.searchByTitleOrAuthors() mock
		Mockito.when(bookService.searchByTitleOrAuthors(title, author)).thenReturn(Arrays.asList(bookTo1));
		// when
		ResultActions response = this.mockMvc.perform(post("/books-search-to/")
				.accept(MediaType.APPLICATION_JSON).contentType(MediaType.APPLICATION_JSON).content(jsonSearchTo.getBytes()));
